...
public class ControlPanel
   ...
   */
  private TshProcessModelTO processModel;
   ...
   */
  @Inject
  public ControlPanel(@Assisted TshProcessModelTO processModel,
  ...
  private void initGuiContent() {
    ...
    for (TshProcessModel.Attribute attribute : TshProcessModel.Attribute.values()) {
      processModelChange(attribute.name(), processModel);
    }
  }
  ...
  public void processModelChange(String attributeChanged, VehicleProcessModelTO newProcessModel) {
    if (!(newProcessModel instanceof TshProcessModelTO)) {
      return;
    }
    processModel = (TshProcessModelTO) newProcessModel;
    ...
    else if (Objects.equals(attributeChanged, TshProcessModel.Attribute.VEHICLE_HOST.name())) {
      updateVehicleHost(processModel.getVehicleHost());
    }
    else if (Objects.equals(attributeChanged, TshProcessModel.Attribute.VEHICLE_PORT.name())) {
      updateVehiclePort(processModel.getVehiclePort());
    }
    else if (Objects.equals(attributeChanged,
                            TshProcessModel.Attribute.VEHICLE_IDLE_TIMEOUT.name())) {
      updateVehicleTimeout(processModel.getVehicleIdleTimeout());
    }
    else if (Objects.equals(attributeChanged, TshProcessModel.Attribute.VEHICLE_IDLE.name())) {
      updateVehicleIdle(processModel.isVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            TshProcessModel.Attribute.DISCONNECTING_ON_IDLE.name())) {
      updateDisconnectingOnIdle(processModel.isDisconnectingOnVehicleIdle());
    }
    else if (Objects.equals(attributeChanged,
                            TshProcessModel.Attribute.RECONNECTING_ON_CONNECTION_LOSS.name())) {
      updateReconnectingOnConnectionLoss(processModel.isReconnectingOnConnectionLoss());
    }
    else if (Objects.equals(attributeChanged, TshProcessModel.Attribute.LAST_ORDER.name())) {
      updateLastOrder(processModel.getLastOrderSent(), processModel.isCommAdapterConnected());
    }
    else if (Objects.equals(attributeChanged,
                            TshProcessModel.Attribute.LOGGING_ENABLED.name())) {
